...
class WrappedLayer(nn.Module):
    ...
    def __init__(self, layer, *args, **kwargs):
        ...
        # register gradient hooks that mask the gradients for sparse training 
        # during dense phases the masks are all 1 so the hooks do not have any effect on the gradients
        self.mask_grad()
    ...
    @property
    def layer_grad_sparsity(self):
        """
        Property which contains current sparsity level for gradients in a wrapped layer
        """
        nz_weight_grad = num_zero(self.weight.grad)
        total_weight_grad = num_total(self.weight.grad)
        nz_bias_grad = 0
        if hasattr(self._layer, 'bias') and (self.bias is not None):
            nz_bias_grad = num_zero(self.bias.grad)
            total_bias_grad = num_total(self.bias.grad)
            total_weight_grad += total_bias_grad
        return nz_weight_grad, nz_bias_grad, total_weight_grad
    ...
    def mask_grad(self):
        ...
        self.mask_grad_hooks = []
        # not necessary to mask bias grads as biases are never pruned
        # also, need to fix the inconsistency of bias masks initially set to 1 and then to None during pruning
        # this causes a bug when the bias hook is used
        #if (self.bias is not None) and (self.bias_mask is not None):
        #    h_b = self.bias.register_hook(self.build_mask_bias_grad_hook())
        #    self.mask_grad_hooks.append(h_b)
        h_w = self.weight.register_hook(self.build_mask_weight_grad_hook())
    ...
    def build_mask_weight_grad_hook(self):
        """
        Builds gradient masking hook
        """
        hook = lambda grad: grad.mul(self.weight_mask)
        return hook
    
    def build_mask_bias_grad_hook(self):
        """
        Builds gradient masking hook
        """
        hook = lambda grad: grad.mul(self.bias_mask)
        return hook
